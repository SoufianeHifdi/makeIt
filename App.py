...
class FileGeneratorApp:
    ...
    def add_input_line(self):
        ...
        labels = ["Number of Files", "Size of Files (KB)"]
        ...
        entries.append(file_type_dropdown)
    ...
    def generate_files(self):
        # Gather user inputs into a data structure for backend use
        inputs = self.get_inputs()
        print(inputs)  # For demonstration, print the inputs to the console
        FileGeneratorDispatch.generate(inputs)

    def get_inputs(self):
        inputs = []
        for entries in self.input_rows:
            # Entries structure is [number_entry, size_entry, type_var, None] (\
            number_of_files = entries[0].get()  # Get number of files input
            size_of_files = entries[1].get()  # Get size of files input
            file_type = entries[2].get()  # Get selected file type from the dropdown

            # Use the globally selected path
            path = self.selected_path.get()

            # Construct a dictionary for this line of input and append it to the inputs list
            input_dict = {
        ...
        return inputs
        # At this point, 'inputs' is a list of dictionaries with all user inputs
        # Here you can process these inputs further, send them to your backend, etc.
